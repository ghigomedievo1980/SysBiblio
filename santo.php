<?php
    ...
    $hoje = date("m-d");
    $sql = "SELECT nomesanto, datasanto FROM santos WHERE DATE_FORMAT(datasanto, '%m-%d') = '$hoje' ORDER BY nomesanto";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo '<li>'.$row["nomesanto"].'</li>';
        }
    } else {
        echo "Nenhum Nome de Santo para hoje.";
    }
